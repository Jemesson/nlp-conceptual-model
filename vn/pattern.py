...
from vn.utility import Printer, WeightedToken, get_case, is_sublist, flatten
...
class PatternIdentifier:
    ...
    def identify_compound(self, story):
        ...
        if story.means.free_form:
            if type(story.means.compounds) is list and len(story.means.compounds) > 0 and type(
                    story.means.compounds[0]) is list:
        ...
        if story.ends.free_form:
            if type(story.ends.compounds) is list and len(story.ends.compounds) > 0 and type(
                    story.ends.compounds[0]) is list:
        ...
        if compounds:
            ...
            for c in compounds:
                self.relationships.append(
                    [story.number, [self.getwt(c[0]), self.getwt(c[1])], Pattern.parent, self.getwt(c[1])])

                ## R4
                if c[0].head == c[1]:
                    self.relationships.append(
                        [story.number, self.getwt(c[0]), Pattern.compound_has, [self.getwt(c[0]), self.getwt(c[1])],
                         self.getwt(c[1])])
    ...
    def identify_func_role(self, story):
        ...
        if story.role.functional_role.compound:
            ...
            if not type(story.role.functional_role.main) is list:
